...
if 'TPU_ADDRESS' in os.environ:
    tpu_address = 'grpc://' + os.environ['TPU_ADDRESS']
...
#    with tf.Session(config=config) as sess:
    with tf.Session(tpu_address) as sess:
        ...
        summary_log = tf.summary.FileWriter(CHECKPOINT_DIR + '/' + args.run_name)
        ...
        if args.restore_from == 'latest':
            ckpt = tf.train.latest_checkpoint(CHECKPOINT_DIR + '/' + args.run_name)
            if ckpt is None:
                # Get fresh GPT weights if new run.
                ckpt = tf.train.latest_checkpoint(MODELS_DIR + '/' + args.model_name)
...
#            saver.save(sess,CHECKPOINT_DIR + "/" + args.run_name,global_step=counter)
            saver.save(sess,CHECKPOINT_DIR + "/" + args.run_name + ".ckpt")
